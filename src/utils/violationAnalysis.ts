...
// Map violations to regulations based on type and industry
export const mapViolationsToRegulations = async (
  ...
  return detections.map(detection => {
    ...
    if (label.includes('hardhat') || label.includes('helmet')) {
      regulations.push({
        ...
        relevance: detection.confidence || 0.8
      });
    } 
    else if (label.includes('vest') || label.includes('visibility')) {
      regulations.push({
        id: '29 CFR 1926.201',
        title: 'Signaling and high-visibility garments',
        relevance: detection.confidence || 0.8
      });
    }
    else if (label.includes('guardrail') || label.includes('railing')) {
    ...
    // Add a general regulation based on industry
